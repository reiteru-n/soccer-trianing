...
export default function KanagawaPage() {
  ...
  const pointsIframeRef = useRef<HTMLIFrameElement>(null);
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      ...
      if (e.data?.type === 'pointsIframeHeight' && pointsIframeRef.current) {
        pointsIframeRef.current.style.height = e.data.height + 'px';
      }
    }
  ...
  return (
    <div className="min-h-screen bg-white pb-16">
      ...
      {/* Tab 2: 総合ポイント推移（iframe） */}
      <div style={{ display: activeTab === 'points' ? 'block' : 'none' }}>
        <iframe
          ref={pointsIframeRef}
          src="/kanagawa-points.html"
          width="100%"
          height="800"
          scrolling="no"
          style={{ border: 'none', display: 'block', overflow: 'hidden' }}
          title="神奈川U-12 総合ポイント推移グラフ"
        />
      </div>
