...
type ColorDef = { bg: string; border: string; badge: string; text: string };
const PART_COLORS: Record<LiftingPart, ColorDef> = {
  ...
  '胸→足':       { bg: 'bg-rose-50',   border: 'border-rose-200',   badge: 'bg-rose-500',   text: 'text-rose-700' },
};
  ...
  return (
    <div className="space-y-3">
      {activeParts.map((part) => {
        ...
        const sideStats = NO_SIDE_PARTS.includes(part)
          ? [{ side: '-', max: partMax, count: partRecords.length }]
          : SIDES.map((side) => {
              const sideRecords = partRecords.filter((r) => r.side === side);
              if (sideRecords.length === 0) return null;
              return { side, max: Math.max(...sideRecords.map((r) => r.count)), count: sideRecords.length };
            }).filter(Boolean);
