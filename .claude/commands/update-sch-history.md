...
### Step 3: 検証エージェントを起動（A・B・C の結果が出揃ってから）

#### エージェント D — データ検証
```
以下はSCH.FC（横浜市泉区）の大会成績リサーチ結果です。
各出典URLに対して WebFetch でアクセスし、記載された成績が実際に正しいかを
一件ずつ確認してください。

【確認観点】
1. 出典URLのページを実際に開き、SCH.FCの成績が記事の内容と一致するか
2. 記事にSCH.FCが登場するか（別チームと混同していないか）
3. 年度・回次・成績（順位/ベスト何）に誤りがないか
4. 404やリンク切れになっていないか

【リサーチ結果】
（※ここにエージェントA・B・Cの結果を貼り付ける）

問題が見つかった場合は「修正が必要」として具体的な誤りと正しい情報を示すこと。
調査のみ行い、コードは書かないこと。
```

> **なぜ必要か**: 前回の作業で検証エージェントが「バディーSC戦のスコア欠落」「重複出典URL」「成績の誤記」を発見した。リサーチエージェントは大量に検索するため誤りを含む可能性がある。

### Step 4: 差分チェック

A・B・C・D のエージェント結果を突き合わせて:
- **新規データ**（現在のページにない年度・成績）を特定する
- **修正が必要なデータ**（D が指摘した誤り）を特定する
